...
//        String udid = PreferencesUtils.getString(context, "udid", "");
//        if (udid.length() == 0) {
//            udid = String.format("%s", UUID.randomUUID());
//            PreferencesUtils.putString(context, "udid", udid);
//        }
//        return udid;
//    }
    ...
    public static boolean isTablet(Context context) {
        if (_isTablet == null) {
            boolean flag;
            if ((0xf & context.getResources()
                    .getConfiguration().screenLayout) >= Configuration.SCREENLAYOUT_SIZE_LARGE)
                flag = true;
            else
                flag = false;
    ...
    public static void gotoMarket(Context context, String pck) {
        if (!isHaveMarket(context)) {
            ToastUtils.show(context, "你手机中没有安装应用市场！");
            return;
        }
        Intent intent = new Intent();
        intent.setAction(Intent.ACTION_VIEW);
     ...
     * @param context
     * @param packageName  app包名
     * @param activityName app Activity名
     * @return
     */
